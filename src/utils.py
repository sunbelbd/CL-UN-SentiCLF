...
def masked_select(input, mask):
    ...
    """
    mask_cast = fluid.layers.cast(x=mask, dtype=input.dtype)
...
def concat_batches(x1, len1, lang1_id, x2, len2, lang2_id, pad_idx, eos_idx, reset_positions):
    ...
    x = np.full(shape=(slen, bs), fill_value=pad_idx)

    x[:len1.max().item()] = x1
    # positions = torch.arange(slen)[:, None].repeat(1, bs).to(x1.device)
    positions = np.arrange(slen)[:, None].repeat(1, bs)
    # langs = x1.new(slen, bs).fill_(lang1_id)
    langs = np.full(shape=(slen, bs), fill_value=lang1_id)
    ...
    assert (x == eos_idx).cast(np.int64).sum().item() == (4 if reset_positions else 3) * bs
