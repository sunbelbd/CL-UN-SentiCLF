...
# All dataset use numpy to dump paddle dependency
...
class StreamDataset(object):
    ...
    def __init__(self, sent, pos, bs, params):
        ...
        # self.lengths = fill_constant(shape=[bs], dtype="int64", value=bptt)
        self.lengths = np.full(shape=[bs], dtype=np.int64, fill_value=bptt)
    ...
    def get_iterator(self, shuffle, subsample=1):
        ...
        for i in indexes:
            a = self.bptt * i
            b = self.bptt * (i + 1)
            # yield to_variable(self.data[a:b].astype(np.int64)), self.lengths
            yield self.data[a:b].astype(np.int64), self.lengths
...
class Dataset(object):
    ...
    def __init__(self, sent, pos, params):
        """
        :param sent: list of token ids. len(sent) is sum of all sentence lengths plus end-of-sentence tokens
        :param pos: list of tuples [(start, end)] of each sentence, sent[pos[i][0]:pos[i][1]+1] is the ith sentence.
        ...
        self.sent = sent
        self.pos = pos
        # get sentences lengths, self.pos[:, 1] are eos indices, self.pos[:, 0] are the start indices
    ...
    def batch_sentences(self, sentences):
        ...
        # when computing slen, don't forget beginning and ending eos tags.
        # lengths = torch.LongTensor([len(s) + 2 for s in sentences])
        lengths = np.array([len(s) + 2 for s in sentences], dtype=np.int64)
        # space holder for batched sentences: (slen, n)
        # sent = torch.LongTensor(lengths.max().item(), lengths.size(0)).fill_(self.pad_index)
        # Use numpy array for safety
        sent = np.full(shape=[lengths.max().item(), lengths.shape[0]], dtype=np.int64, fill_value=self.pad_index)
        # sent[0] is always the end of sentence (eos)
        sent[0] = self.eos_index
        for i, s in enumerate(sentences):
            if lengths[i] > 2:  # if sentence not empty
                # sent[1:lengths[i] - 1, i].copy_(torch.from_numpy(s.astype(np.int64)))
                sent[1:lengths[i] - 1, i] = s.astype(np.int64)
            # put eos tag at the end of sentence
    ...
    def trim_long_sentences(self, max_len):
        """
        Trim sentences exceeding a certain length.
        Currently only implemented for Dataset class. ParallelDataset class
        ...
        if max_len == 0:
            return
        indices = np.arange(len(self.pos))
        indices_long = indices[self.lengths[indices] > max_len]
        # shift eos forward
        self.sent[self.pos[indices_long, 0] + max_len] = self.eos_index
        self.sent[self.pos[indices_long, 1]] = -1
        self.pos[indices_long, 1] = self.pos[indices_long, 0] + max_len
        self.lengths[indices_long] = max_len
        logger.info("Trimmed %i too long sentences." % (len(indices_long)))
        self.check()
        return indices
    ...
    def get_iterator(self, shuffle, group_by_size=False, n_sentences=-1, seed=None, return_indices=False):
        """
        Return a sentences iterator.
        :param shuffle: shuffle input or not. Set to true only for training
        :param group_by_size:
        :param n_sentences: max # sentences for iterator. -1 means full data
        :param seed:
        :param return_indices: return sentence indices or not. Useful for supervised learning
        when there is a label vector. The returned indices can be used to slice corresponding
        target vector for input batch.
        :return: an iterator over sentence indices
        """
        ...
        # sentence lengths, plus 2 each to hold beg, end EOS tags
        ...
        if self.tokens_per_batch == -1:
            # split indices into number of batches
        ...
        # return the iterator on batches of indices
