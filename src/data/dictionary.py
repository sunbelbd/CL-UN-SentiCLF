...
class Dictionary(object):
    ...
    @staticmethod
    def merge_vocab(vocab1_path, vocab2_path, bpe1_path, bpe2_path):
        """
        Create a new dictionary from the intersections of dico1 and dico2, update
        vocabulary and BPE codes. Newly generated vocab and BPE codes will be used to index data.
        """

        def load_bpe(bpe_path):
            skipped = 0
            assert os.path.isfile(bpe_path)
            f = open(bpe_path, 'r', encoding='utf-8')
            bpe2cnts = defaultdict(int)
            for i, line in enumerate(f):
                line = line.rstrip()
                ...
                # assert line[0] not in word2id and line[1].isdigit(), (i, line)
                assert line[1].isdigit(), (i, line)
                if line[0] in bpe2cnts:
                    skipped += 1
                    print('%s already in BPE code' % line[0])
                    continue
                if not line[1].isdigit():
                    skipped += 1
                    print('Empty BPE code at line %s with count %s' % (i, line))
                    continue
                bpe2cnts[line[0]] = int(line[1])
            f.close()
            return bpe2cnts

        def write_dict(out_dict, file_name):
            f = open(file_name, 'w', encoding='utf-8')
            for k, v in sorted(out_dict.items(), key=lambda x: -x[1]):
                if v > 0:
                    f.write("%s %d\n" % (k, v))
            f.flush()
            f.close()

        dico1, dico2 = Dictionary.read_vocab(vocab1_path), Dictionary.read_vocab(vocab2_path)
        print("source vocab size: %d, target vocab size: %d" % (len(dico1), len(dico2)))
        bpe1, bpe2 = load_bpe(bpe1_path), load_bpe(bpe2_path)
        print("source bpe size: %d, target bpe size: %d" % (len(bpe1), len(bpe2)))
        vocab = {k: v for k, v in dico1.counts.items() if k in dico2.counts and v > 0}
        bpe = {k: v for k, v in bpe1.items() if k in bpe2 and v > 0}
        print("merged vocab size: %d, merged bpe size: %d" % (len(vocab), len(bpe)))
        # output vocab and bpe files
        write_dict(vocab, vocab1_path + "_merged")
        write_dict(bpe, bpe1_path + "_merged")
    ...
    def index_data(path, bin_path, dico):
        ...
        for i, line in enumerate(f):
            ...
            for w in s:
                ...
                # if we find a special word which is not an unknown word, skip the word
